...
void test_3DST()
{
  ...
  PredictionNoExtrap predNumuPID(loaderNonswap, loaderNue, loaderNuTau, "PID", Binning::Simple(60, -.1, +1.1), SIMPLEVAR(dune.cvnnumu) ,1 , kNoCut,map, kNoShift);

  PredictionNoExtrap predNuePID(loaderNonswap, loaderNue, loaderNuTau, "PID", Binning::Simple(60, -.1, +1.1), SIMPLEVAR(dune.cvnnue) ,1 , kNoCut, map, kNoShift);

  //SpectrumLoaderBase& dummyLoaders = kNullLoader;

  // offLocation amd pre-loaded map are NOT USED!
  DUNENoExtrapPredictionGenerator genFDNumuFHC(loaderNonswap,
                                                loaderNue,
                                                loaderNuTau,
                                                loaderNuTau,
                                                "Neutrino Energy (GeV)", Binning::Simple(38, 0.5, 10), SIMPLEVAR(dune.Ev),
                                                SIMPLEVAR(dune.nuPDG) == 14, kUnweighted);

  PredictionInterp pred(useSysts, calc0, genFDNumuFHC, dummyLoaders);

  //PredictionNoExtrap pred(loaderNonswap, loaderNue, loaderNuTau, HistAxis( "Neutrino Energy (GeV)", Binning::Simple(40, 0, 10), SIMPLEVAR(dune.Ev)), SIMPLEVAR(dune.nuPDG) == 14 , map, kNoShift, 1);

  //PredictionNoExtrap pred(loaderNonswap, loaderNue, loaderNuTau, "Reconstructed E (GeV)", Binning::Simple(80, 0, 10), Enu_recoNumu, 1, kNoCut, map, kNoShift);
  ...
  PredictionNoExtrap predNue(loaderNonswap, loaderNue, loaderNuTau, "Reconstructed E (GeV)", Binning::Simple(24, 0, 6), Enu_recoNue, 1, kSelNue, map, kNoShift);
